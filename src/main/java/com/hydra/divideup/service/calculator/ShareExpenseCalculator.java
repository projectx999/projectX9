...
public final class ShareExpenseCalculator implements ExpenseCalculator {
  ...
  public List<Expense> calculateExpenses(Payment payment) {
    ...
    var totalShares = BigDecimal.valueOf(payment.getSplitDetails().values().stream()
    ...
    var expenses = payment.getSplitDetails().entrySet().stream()
            .filter(entry -> !entry.getKey().equals(payment.getPaidBy()))
            .map(entry -> {
              var shareAmount = sharePrice.multiply(BigDecimal.valueOf(entry.getValue())).negate();
              return new Expense(payment, entry.getKey(), shareAmount.stripTrailingZeros());
            })
            .collect(Collectors.toList());

    // Create expenses for the payer
    var payerAmount = expenses.stream()
